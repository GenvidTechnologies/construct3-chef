...
describe("addonInventory", () => {
  ...
  it("classifies bundled = declared in usedAddons AND on disk", () => {
    expect(idsWithStatus(FIXTURE_ROOT, "bundled")).to.deep.equal([
      "CleanControl",
      "Complete",
      "Dup",
      "NameMismatchBehavior",
  ...
  describe("formatAddonInventory", () => {
    ...
    it("renders per-status line shapes", () => {
      ...
      expect(lines[0]).to.equal("12 addon(s):");
      ...
      expect(lines).to.include(
        "  NameMismatchBehavior  bundled  1.0.0.0  addons/behavior/NameMismatchBehavior.c3addon",
      );
