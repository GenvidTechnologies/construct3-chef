...
describe("recipeInterpreter", () => {
  ...
  describe("expandEvent", () => {
    ...
    it("throws for unrecognized shorthand", () => {
      assert.throws(
        () => expandEvent(sidGen, { foo: "bar" } as unknown as BuilderEvent),
        /Unrecognized event shorthand/,
      );
    });
  });
  ...
  describe("executeOp: rename-symbol", () => {
    ...
    it("skips non-script actions (standard, call, comment)", () => {
      const stdAction = buildAction(sidGen, {
        id: "set-text",
        objectClass: "OldModule",
        parameters: { text: "OldModule.foo" },
      });
      ...
      executeFileOps(sidGen, sheet, [
        {
          ...
          in: "sid:10",
          after: 0,
          block: { conditions: [], actions: [{ script: ["// after-0"] }] },
        },
      ]);
      const children = (sheet.events[0] as BlockEvent).children as BlockEvent[];
  ...
  describe("validateRecipe: shorthand field validation", () => {
    ...
    it("rejects unknown field on an insert-actions action shorthand", () => {
      const recipe = {
        files: {
          "Test/Sheet": [
            { op: "insert-actions", in: "sid:1", after: 0, actions: [{ id: "destroy", object: "Foo", objclass: "x" }] },
          ],
        },
      } as unknown as Recipe;
      const errors = validateRecipe(recipe);
      assert.ok(
        errors.some((e) => e.includes('unknown field "objclass"')),
        errors.join("; "),
      );
    });
    ...
    it("accepts objectClass alias on an insert-actions action shorthand", () => {
      const recipe = {
        files: {
          "Test/Sheet": [
            { op: "insert-actions", in: "sid:1", after: 0, actions: [{ id: "destroy", objectClass: "Foo" }] },
          ],
        },
      } as unknown as Recipe;
      const errors = validateRecipe(recipe);
    ...
    it("rejects an insert-actions action with no object (non-System)", () => {
      ...
      const errors = validateRecipe(recipe);
      assert.ok(
        errors.some((e) => e.includes('missing "object"')),
        errors.join("; "),
      );
    });
    ...
    it("allows a System action id with no object in insert-actions", () => {
      const recipe = {
        files: {
          "Test/Sheet": [
            { op: "insert-actions", in: "sid:1", after: 0, actions: [{ id: "wait-for-previous-actions" }] },
          ],
        },
      } as unknown as Recipe;
      const errors = validateRecipe(recipe);
    ...
    it("rejects an insert-actions custom-action with no object (no validate/apply mismatch)", () => {
      const recipe = {
        files: {
          "Test/Sheet": [{ op: "insert-actions", in: "sid:1", after: 0, actions: [{ "custom-action": "Foo" }] }],
        },
      } as unknown as Recipe;
      const errors = validateRecipe(recipe);
      assert.ok(
          ...
          "eventSheets/Test/Test.json": [
            {
              ...
              action: {
                ...
                object: "System",
                params: { layer: "BadLayer", visibility: "visible" },
              },
            } as unknown as FileOp,
          ],
        },
          ...
          "eventSheets/Test/Test.json": [
            {
              ...
              block: {
                conditions: [{ id: "is-on-layer", object: "Sprite", params: { layer: "BadLayer" } }],
                actions: [
                  { id: "set-layer-visible", object: "System", params: { layer: "BadLayer", visibility: "0" } },
                ],
              },
            } as unknown as FileOp,
          ],
  ...
  describe("validateActionParams: callFunction params check", () => {
    it("R3.3: warns when call params is an object", () => {
      const warnings = validateActionParams({ call: "myFunc", params: { "0": "arg1", "1": "arg2" } }, "test");
      assert.equal(warnings.length, 1);
      assert.include(warnings[0], '"params" must be an array');
    });

    it("R3.4: no warning when call params is an array", () => {
      const warnings = validateActionParams({ call: "myFunc", params: ["arg1", "arg2"] }, "test");
      assert.deepStrictEqual(warnings, []);
    });
  });
