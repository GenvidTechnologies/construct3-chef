...
> `CLAUDE.md` § "Where to read more", not here: durable knowledge with *no other
> repo home*. Knowledge already owned by `CLAUDE.md`, `docs/`, or an ADR stays
> there; a second copy is a drift trap, not a convenience.
>
...
- A page whose claims track a **pinned upstream version** (`@genvidtech/c3source`,
  ...
  `docs/leaf-dependency-ledger.md` already owns that axis, so a wiki page
  restating it is the duplication the routing rule forbids.
- A page about a **settled decision** omits `stale_after` entirely — but such a
  page usually belongs in `docs/decisions/` rather than here.
