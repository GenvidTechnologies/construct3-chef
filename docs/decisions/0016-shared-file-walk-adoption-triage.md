...
- **The regression tests for site 1 are split by dimension** (a `*.uistate.json` sibling; a `uistate/` subfolder) rather than combined, because `EDITOR_LOCAL_EXCLUSIONS` treats file suffixes and directory names as independent mechanisms and a combined test could not say which one regressed. They are synthetic by necessity — the `construct3-chef-sample` fixture carries zero uistate files, so a fixture-based version would pass vacuously.
