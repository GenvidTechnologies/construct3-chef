...
Design rationale and research behind construct3-chef's MCP server (`src/mcp/server.ts`) and the file-based model it wraps. This is the durable reference; the operational summary lives in [CLAUDE.md](../CLAUDE.md) (§ "The two-surface data model", § "MCP server state model"), and the tool-by-tool usage lives in [recipe-reference.md](recipe-reference.md), [generators.md](generators.md), [cli.md](cli.md), and [cli-addons.md](cli-addons.md).
