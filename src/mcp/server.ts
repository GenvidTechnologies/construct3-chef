...
reg(
  "scan-addon-usage",
  {
    ...
    description:
      "Scan the project for usage of a plugin, behavior, or effect addon. For plugin/behavior addons: which object types/families are instantiated from the addon (presence), and which conditions/actions on them call one of its current ACEs (call sites), grouped by event sheet. For effect addons (which have no ACEs): every site the effect is applied — object types, families, layers, and layouts — since presence is the whole story. With `from` (a .c3addon file path, or a discovered addon id/dir for a prior version), also reports blast radius — for plugins/behaviors, call sites whose ACE was changed or removed between `from` and the addon's current ACEs (surfacing dangling references a reimport may not have migrated); for effects, every application site (a version bump can change any effect parameter). Read-only.",
    annotations: READ_ONLY,
    inputSchema: {
      addon: z
        .string()
