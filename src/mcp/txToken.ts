import { mcpError, formatTxToken, parseTxToken, type TxTokenParseFailure } from "@genvidtech/mcp-utils";
import type { CallToolResult } from "@modelcontextprotocol/sdk/types.js";
import type { ProjectContext } from "./projectContext.js";

/**
 * Composite `<projectId>:<counter>` txId policy + rendering (#95, ADR
 * `wiki/decisions/0034`; codec adopted from upstream, #217). Under
 * single-project MCP the txId was a bare integer; under multi-project two
 * projects sitting at the same counter value is the common case, not a
 * coincidence (both start at 0 and increment per mutation), so a bare
 * integer from project alpha would be silently accepted against project
 * beta. The composite token makes it self-describing so a cross-project
 * token is a detectable error rather than a silent accept.
 *
 * `formatTxToken`/`parseTxToken` — the codec itself — now come from
 * `@genvidtech/mcp-utils` (re-exported below for `formatTxToken`, imported
 * directly for `parseTxToken`); this module owns comparison policy
 * ({@link compareTxToken}) and chef's client-facing rendering of upstream's
 * parse-failure reasons ({@link renderParseFailure}).
 *
...
// Re-export of upstream's codec (#217) — `server.ts` imports `formatTxToken`
// from this module and must keep doing so unchanged. Project ids may not
// contain `:` or whitespace, and may not be empty — enforced at
// `ProjectRegistry.add` through `@genvidtech/mcp-utils`' `isValidProjectId`,
// the same rule this wire format's other consumers apply.
export { formatTxToken };
 ...
 *
 * Wired into {@link compareTxToken}'s parse-failure branch (#217).
 */
 ...
 *   optionality.
 * - malformed token (fails upstream's `parseTxToken`) -> error rendered by
 *   {@link renderParseFailure} from the returned `reason`.
...
export function compareTxToken(ctx: ProjectContext, token?: string, action?: string): CallToolResult | null {
  ...
  if (!parsed.ok) {
    return mcpError(renderParseFailure(token, parsed.reason), footer);
  }
  if (parsed.projectId !== ctx.id) {
    return mcpError(
      `txId '${token}' is for project '${parsed.projectId}' but this call targets project '${ctx.id}'`,
      footer,
    );
  }
  if (parsed.n !== ctx.watcher.txId) {
