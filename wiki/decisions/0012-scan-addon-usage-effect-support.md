...
title: "0012. `scan-addon-usage`: effect addon support"
description: >-
  `scan-addon-usage` extended to effect addons via a dedicated `scanEffectUsage` path (not a `UsageMatcher` extension, course-correcting ADR 0011's prediction); the four-site presence model (object type/family/layer/layout); presence-only, `--from` blast = every application site ([#125](https://github.com/GenvidTechnologies/construct3-chef/issues/125), part of the #100 c3addon-tooling umbrella)
