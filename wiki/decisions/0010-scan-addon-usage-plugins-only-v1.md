...
title: "0010. `scan-addon-usage`: plugins-only v1 scope"
description: >-
  `scan-addon-usage` scans plugin ACE call sites in event sheets only; behavior/effect/expression usage split into follow-ups (#123/#124/#125); `readProjectObjects`/`ObjectDefn` shared seam; blast-radius match-set widening for dangling calls to removed ACEs ([#110](https://github.com/GenvidTechnologies/construct3-chef/issues/110), part of the #100 c3addon-tooling umbrella)
...
- Blast mode's match-set widening (`currentAceSet ∪ removedKeys`) is the one
  ...
  `wiki/reference/cli-addons.md` so it isn't mistaken for an implementation detail that
