...
title: "0026. Split the leaf-dependency version narrative out of `CLAUDE.md`"
description: >-
  `CLAUDE.md`'s version-by-version upstream narrative moves to `wiki/process/leaf-dependency-ledger.md` (one section per package, oldest entry first), leaving only the adoption posture in `CLAUDE.md` § "Leaf dependencies"; rejects `CHANGELOG.md` as the home (different axis and lifecycle) and strictly-chronological single-file ordering (interleaving is what let the mcp-utils 0.6.0 entry go missing); the deliberate declines move with their version entries, reachable via their indexed ADRs and a routing pointer left in `CLAUDE.md` ([#172](https://github.com/GenvidTechnologies/construct3-chef/issues/172))
...
Move the narrative to a new `wiki/process/leaf-dependency-ledger.md`, one `##`
...
- `wiki/index.md` gains a `leaf-dependency-ledger.md` entry under
