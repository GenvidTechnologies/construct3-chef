...
title: "0027. Cover a Windows-only defect with cross-platform stub coverage, not manual verification"
description: >-
  Mcp-utils 0.7.0's `OptimisticWatcher` Layer 3 (Windows/NTFS duplicate-`fs.watch`-event fix) is covered by injected-stub-factory tests instead of the issue's proposed manual Windows run, on the rule that a platform-specific trigger does not imply platform-specific testability; rejects manual verification as the merge gate, relying on the unmodified pre-existing suite (vacuous — every prior test fires its path once), and `observed: null` (forfeits the fix); records the accepted gap that stub coverage proves the collapsing behaviour but not the native duplicate-delivery trigger itself ([#191](https://github.com/GenvidTechnologies/construct3-chef/issues/191))
...
- The accepted gap this decision leaves open: nothing locally confirms
  ...
  `wiki/process/leaf-dependency-ledger.md`'s `0.7.0` entry as the thing to
